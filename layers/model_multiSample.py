...
class MultiSampleBinaryLayer(nn.Module):
    def __init__(self, input_dim, output_dim, sampleQ=1, new_loss_importance = 0.1):
    ...
    def forward(self, x, with_grad=True):
        ...
        with torch.no_grad():
            p = torch.sigmoid(l)
        targetSize = tuple([self.SampleQ] + list(p.size()))
        o = torch.bernoulli(p.expand(targetSize))
        if with_grad:
            grad_cor = o - p
            with torch.no_grad():
            ...
            torch.sum(l*torch.sum(grad_cor, 0)).backward()
        return torch.sum(o, 0)
...
def demonstrate():
    sl = MultiSampleBinaryLayer(2, 3).cuda()
